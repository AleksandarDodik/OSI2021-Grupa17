...
void Racun::dodavanjeArtikala(std::string& imeRadnika)
{
	...
	while (i==1)
	{
		...
		if (naziv.compare("RACUN") == 0)
		{
			...
			kreiranjeRacuna(atof((popust).c_str()), atof((uplata).c_str()), imeRadnika);
			ispisRacuna();
		}
...
void Racun::ispisRacuna()
{
	system("cls");
	std::string temp;
	std::ifstream Racun;
	Racun.open("Racun1.txt");
	if (Racun.is_open())
	{
		while (!Racun.eof())
		{
...
void Racun::kreiranjeRacuna(double popust,double uplata,std::string& imeRadnika)
{
	std::ofstream Racun;
	Racun.open("Racun1.txt", std::ios::trunc);
	std::ifstream Konfig;
	std::string a, b, c;
	int sirina;
